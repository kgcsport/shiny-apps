...
db_exec("CREATE TABLE IF NOT EXISTS round_absences(
...
ensure_column("job_posts", "description TEXT")
...
ensure_column("job_templates", "description TEXT")
...
assignment_round_for_timing <- function(current_round_id, timing_filter) {
  # Timing controls when work is assigned/revealed, not which lecture it
  # belongs to. Notes assigned at the end of class document the lecture that
  # just ended.
...
reveal_timings_for_scope <- function(scope) {
  ...
  else if (identical(scope, "end")) "end"
  else "start"
}

job_description_details <- function(description, label = "What to do") {
  description <- trimws(as.character(description %||% ""))
  if (!nzchar(description)) return(NULL)
  tags$details(
...
server <- function(input, output, session) {
  ...
  observeEvent(input$login_btn, {
    u <- norm_username(input$login_user %||% "")
    p <- input$login_pw %||% ""
        ...
        div(class = "pool-grid",
          lapply(seq_len(nrow(today_posts)), function(i) {
            ...
            div(class = paste("pool-card", if (full) "pool-card-full"),
              ...
              job_description_details(r$description, "Description")
            )
          })
        )
      } else {
  ...
  observeEvent(input$impersonate_uid, {
    ...
    uid <- norm_username(input$impersonate_uid %||% "")
    if (!nzchar(uid)) return()
    row <- db_query(
      "SELECT user_id, display_name, course, section, COALESCE(is_demo,0) AS is_demo
       FROM users WHERE LOWER(user_id)=LOWER(?) AND COALESCE(active,1)=1;", list(uid))
    if (!nrow(row)) return()
  ...
  observeEvent(input$save_student_btn, {
    ...
    new_uid <- norm_username(input$edit_student_uid %||% "")
    ...
    if (!identical(tolower(old_uid), tolower(new_uid))) {
      exists <- tryCatch(db_query(
        "SELECT user_id FROM users WHERE LOWER(user_id)=LOWER(?) AND LOWER(user_id)<>LOWER(?);",
        list(new_uid, old_uid)),
        error = function(e) data.frame())
      if (nrow(exists)) {
  ...
  observeEvent(input$create_student_btn, {
    req(rv$is_admin)
    uid <- norm_username(input$new_stu_uid %||% "")
  ...
  observeEvent(input$reset_pw_btn, {
    req(rv$is_admin)
    uid <- norm_username(input$reset_pw_uid %||% "")
  ...
  observeEvent(input$bulk_upload_students_btn, {
    ...
    for (i in seq_len(nrow(df))) {
      uid  <- norm_username(df[[uid_col]][i] %||% "")
  ...
  observeEvent(input$add_template_btn, {
    ...
    desc   <- trimws(input$new_tpl_desc %||% "")
  ...
  observeEvent(input$edit_job_post_open, {
    ...
    showModal(modalDialog(
      ...
      textAreaInput("edit_post_desc", "Quick description / instructions:",
                    value = post$description[1] %||% "", rows = 3,
                    placeholder = "Example: Post notes to the Moodle forum under the related day's course notes."),
  ...
  mark_cold_call_absent <- function() {
    req(rv$is_admin)
    ...
    db_exec(
      "INSERT OR REPLACE INTO round_absences(round_id,user_id,marked_at)
       VALUES(?,?,CURRENT_TIMESTAMP);",
      list(as.integer(round$id[1]), drawn$user_id))
    absent_name <- drawn$display_name %||% drawn$user_id
    ...
    div(
      ...
      div(style = "display: flex; gap: 6px; flex-wrap: wrap;",
        ...
        if (nzchar(drawn_uid)) tagList(
          ...
          actionButton("slide_cold_call_absent_btn", "Absent & Redraw", class = "btn btn-sm btn-warning"),
  ...
  observeEvent(input$redraw_absent_btn, {
    ...
    db_exec(
      "INSERT OR REPLACE INTO round_absences(round_id,user_id,marked_at)
       VALUES(?,?,CURRENT_TIMESTAMP);",
      list(old$round_id[1], old$user_id[1]))
    db_exec(
            ...
            if (nzchar(drawn_uid)) {
              div(style = "display:flex;gap:.4rem;flex-wrap:wrap;margin-top:.6rem;",
                ...
                actionButton("record_cold_call_board_btn", "Board Work",
                             class = "btn btn-sm btn-outline-success"),
                actionButton("cold_call_absent_btn", "Absent & Redraw",
  ...
  output$config_panel <- renderUI({
    ...
    if (act == "jobs") {
        ...
        if (!is.na(rid)) {
          ...
          tags$details(
            ...
            div(style = "padding:.5rem 0;",
              ...
              textAreaInput("new_post_desc", "Quick description / instructions:",
                            rows = 2, width = "100%",
                            placeholder = "Example: Post notes to the Moodle forum under the related day's course notes.")
            )
          )
      ...
      if (length(section_vals)) {
        scope_df <- data.frame(course = course_vals,
                               section = section_vals,
                               stringsAsFactors = FALSE)
        scope_df <- scope_df[nzchar(scope_df$section), , drop = FALSE]
        scope_df <- scope_df[!duplicated(paste(norm_key(scope_df$course),
                                               norm_key(scope_df$section),
                                               sep = "\r")), , drop = FALSE]
        if (nrow(scope_df)) {
  ...
  observeEvent(input$bulk_award_btn, {
    ...
    targets <- if (identical(scope, "all")) {
      ...
      tryCatch(db_query(
        "SELECT user_id, display_name FROM users
         WHERE COALESCE(is_admin,0)=0 AND COALESCE(active,1)=1 AND COALESCE(is_demo,0)=0
         AND LOWER(course)=LOWER(?);", list(course)),
        error = function(e) data.frame())
      ...
      if (nzchar(course)) {
        tryCatch(db_query(
          "SELECT user_id, display_name FROM users
           WHERE COALESCE(is_admin,0)=0 AND COALESCE(active,1)=1 AND COALESCE(is_demo,0)=0
           AND LOWER(course)=LOWER(?) AND LOWER(section)=LOWER(?);", list(course, section)),
          error = function(e) data.frame())
      } else {
        tryCatch(db_query(
          "SELECT user_id, display_name FROM users
           WHERE COALESCE(is_admin,0)=0 AND COALESCE(active,1)=1 AND COALESCE(is_demo,0)=0
           AND (course IS NULL OR course='') AND LOWER(section)=LOWER(?);", list(section)),
          error = function(e) data.frame())
      }
    } else {
      tryCatch(db_query(
